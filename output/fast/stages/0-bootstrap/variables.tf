  ...
  default = {
    ...
    sql_instance_configuration_changes = {
      filter = <<-FILTER
              ...
              AND protoPayload.methodName=("cloudsql.instances.update" OR "cloudsql.instances.patch" OR "cloudsql.instances.create" OR "cloudsql.instances.delete")
      FILTER
      type   = "logging"
    }
    ...
    vpc_network_route_changes = {
      filter = <<-FILTER
          ...
          AND jsonPayload.event_type="gce.routes.insert" OR jsonPayload.event_type="gce.routes.delete"
      FILTER
      type   = "logging"
    }
    vpc_firewall_rule_changes = {
      filter = <<-FILTER
            ...
            AND jsonPayload.event_type=("gce.firewalls.insert" OR "gce.firewalls.patch" OR "gce.firewalls.delete")
      FILTER
      type   = "logging"
    }
    custom_role_changes = {
      filter = <<-FILTER
          resource.type="iam.googleapis.com/Role"
