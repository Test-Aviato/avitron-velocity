...
variable "log_sinks" {
  ...
  #Add more parameters to log sink
  #   exclusions = optional(map(string), {})
  #      filter               = optional(string)
  #      type = string
  #      destination          = string

   alert_config = optional(map(object({
    audit_configuration_changes = optional(object({
       notification_channels = optional(list(string), [])
      }));
    bucket_permission_changes = optional(object({
       notification_channels = optional(list(string), [])
      }));
    custom_role_changes = optional(object({
       notification_channels = optional(list(string), [])
      }));
    project_ownership_changes = optional(object({
       notification_channels = optional(list(string), [])
      }));
    vpc_firewall_rule_changes = optional(object({
       notification_channels = optional(list(string), [])
      }));
  })),{})
}
...
variable "alert_config" {
  description = "Log metric Filters for Audit Configuration Changes,Cloud Storage Bucket IAM Permission Changes,Custom Role Changes,Project Ownership Assignments and Changes and VPC Network Firewall Rule Changes"
  type = object({
    audit_configuration_changes = object({
       notification_channels = list(string)
      })
    bucket_permission_changes = object({
       notification_channels = list(string)
      })
    custom_role_changes = object({
       notification_channels = list(string)
      })
    project_ownership_changes = object({
       notification_channels = list(string)
      })
    vpc_firewall_rule_changes = object({
       notification_channels = list(string)
      })
  })
  default = {
