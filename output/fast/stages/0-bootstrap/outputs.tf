...
locals {
  ...
  cicd_workflows = {
    for k, v in local.cicd_repositories : "${v.level}-${k}" => templatefile(
      "${path.module}/templates/workflow-${v.repository.type}.yaml", {
        ...
        service_accounts = {
          apply = try(module.automation-tf-cicd-sa[k].iam_email, "")
          plan  = try(module.automation-tf-cicd-r-sa[k].iam_email, "")
        }
        stage_name = k
        tf_providers_files = {
  ...
  tfvars = {
    automation = {
      ...
      service_accounts = {
        bootstrap   = module.automation-tf-bootstrap-sa.iam_email
        bootstrap-r = module.automation-tf-bootstrap-r-sa.iam_email
        resman      = module.automation-tf-resman-sa.iam_email
        resman-r    = module.automation-tf-resman-r-sa.iam_email
        vpcsc       = module.automation-tf-vpcsc-sa.iam_email
        vpcsc-r     = module.automation-tf-vpcsc-r-sa.iam_email
      }
    }
    billing = {
...
output "cicd_repositories" {
  ...
  value = {
    for k, v in local.cicd_repositories : k => {
      ...
      service_account = try(module.automation-tf-cicd-sa[k].iam_email, null)
    }
  }
}
...
output "workforce_identity_pool" {
  ...
  value = {
    pool = try(
      google_iam_workforce_identity_pool.default[0].name, null
    )
  }
}
