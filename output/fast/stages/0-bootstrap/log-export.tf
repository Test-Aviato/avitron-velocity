...
resource "google_project_iam_binding" "project-level-logging" {
  project = module.log-export-project.project_id
  role = "roles/logging.logSinkConfigurationWriter"
  members = [
    "serviceAccount:service-org-${var.organization.id}@gcp-sa-logging.iam.gserviceaccount.com"
  ]
